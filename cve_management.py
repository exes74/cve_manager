...
def download_cves(directory):
	...
	# print(str(r.text))
	for filename in re.findall("nvdcve-1.1-[0-9]*\.json\.gz",r.text):
		print(filename)
...
def process_cves(directory, results, csv, import_db,myuser,mypassword,myhost,database):
	if csv:
		...
		all_cves = []
		directory = directory + "/"
		files = [f for f in listdir(directory) if (isfile(join(directory, f)) and str(f).endswith('gz'))]
		
		# print(str(files))
		files.sort(reverse=True)
		# for file in files:
			# print ("\nDecompressing ", file)
			# with gzip.open(join(directory, file), 'r') as f_in:
				# with open(join(directory, file+'.out'), 'wb') as f_out:
					# shutil.copyfileobj(f_in, f_out)
		files2 = [f for f in listdir(directory) if (isfile(join(directory, f)) and str(f).endswith('out'))]
		for file in files2:
			print ("\nProcessing ", file)
			json_data = open(join(directory, file) , 'r' ,  encoding="utf-8")
			# json_data2 = json_data.read()
			cve_dict = json.loads(fix_text(json_data.read()))
		...
		for cves in all_cves:
			...
			if description.find("** REJECT **"):
				...
				if cvssv2_missing and cvssv3_missing:
					...
					file_cvss_score.write(do_encoding(cve)+";"+cvssv3+";"+do_encoding(cvssv2)+";"+";"+";"+do_encoding(description)+";"+do_encoding(cves['publishedDate'])+";"+do_encoding(cves['lastModifiedDate'])+"\n")
...
if __name__ == '__main__':
	...
	parser.add_argument('-v', '--version',  action="store_true", dest="process", default="0.1", help="Process downloaded CVEs. version")
